# -*- mode: python -*-
import sys, os, string, time, re, pickle, pdb, zipfile, subprocess
import SCons
...
def checkLDD( target, source, env ):
	...
	if (not os.path.isfile(file.abspath)):
		...
		Exit(1)
	( status, output ) = subprocess.getoutput( 'ldd -r %s' % file )
	if ( status != 0 ):
